...
import { HotspotCommon } from './hotspot-common';

@Component({
  templateUrl: 'new-hotspot.component.html'
})
export class NewHotspotComponent extends HotspotCommon implements OnInit, OnDestroy {
  model: Hotspot;
  ...
  save (e) {
    ...
    let pipe_ids = this.selectedFeatures.getArray().map(f => f.getProperties().id);
    formData.append('MsgList', pipe_ids.join(' '));
    formData.append('extent', JSON.stringify(this.selectService.getSelectedFeaturesExtent()));
    formData.append('context', 'ENTER');

    this.http
      .post('//ch.ci.garden-grove.ca.us/cgi-bin/pubworks/hotspot/full/hotspot_input.cgi', formData).toPromise()
